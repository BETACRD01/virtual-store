...
import{s as d}from"./auth.service-BxJS2Ta1.js";import{r as u,_ as g}from"./admin.guard-BNG6i5Q6.js";import{s as l}from"./dom-Dz1M3mpB.js";import{s as p,a as c}from"./notifications-DBxoMB4P.js";const s="/virtual-store/";async function v(){var i;const a=document.getElementById("admin-sidebar");a.innerHTML=`<div class="admin-sidebar">
      ...
      </table></div>`,e.querySelectorAll(".toggle-cat").forEach(t=>t.addEventListener("click",async()=>{const n=t.dataset.id,r=t.dataset.active==="true";await d.from("categories").update({is_active:!r}).eq("id",n),c(r?"Categoría desactivada.":"Categoría activada."),o()})),e.querySelectorAll(".edit-cat").forEach(t=>t.addEventListener("click",()=>{const n=prompt("Nombre:",t.dataset.name);if(!n)return;const r=prompt("Slug:",t.dataset.slug);if(!r)return;const m=prompt("Descripción:",t.dataset.desc)||"";d.from("categories").update({name:n.trim(),slug:r.trim(),description:m}).eq("id",t.dataset.id).then(()=>{c("Categoría actualizada."),o()})}))}document.getElementById("add-cat-btn").addEventListener("click",async()=>{const a=prompt("Nombre de la categoría:");if(!a)return;const i=prompt("Slug (ej: electronica):");if(!i)return;const e=prompt("Descripción (opcional):")||"",{error:t}=await d.from("categories").insert({name:a.trim(),slug:i.trim(),description:e,is_active:!0});if(t){p(t.message);return}c("Categoría creada."),o()});async function h(){await u(),await v(),o()}h();
