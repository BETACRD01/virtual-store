import"./auth.service-C3JQ1Rkv.js";import{l as k,a as S,c as T}from"./footer-DmXOtuA9.js";import{p as v}from"./profile.service-wGikjLhl.js";import{o as j}from"./orders.service-CcZW1sOW.js";import{f}from"./currency-BUz-WTj3.js";import{s as a,d as q,e as z}from"./dom-Dz1M3mpB.js";import{a as E,s as h}from"./notifications-DBxoMB4P.js";import{a as M}from"./auth.guard-BolzWgS9.js";const _="/virtual-store/";let b=1,s=null,o="cash";async function C(){const y=await M();if(!y)return;await k(),S();const p=y.user.id,n=await T.getCart(p);if(!n.length){window.location.href=`${_}pagina/carrito.html`;return}async function i(){var $,w;const c=document.getElementById("checkout-form"),m=document.getElementById("checkout-summary"),d=await v.getAddresses(p),g=n.reduce((e,t)=>{var r,l;return e+(t.quantity||1)*(t.unit_price||((r=t.product)==null?void 0:r.offer_price)||((l=t.product)==null?void 0:l.price)||0)},0),u=g>999?0:99,A=g+u;if(m.innerHTML=`<div class="card" style="padding:1.5rem;position:sticky;top:100px">
          ...
          ${n.slice(0,4).map(e=>{const t=e.product||{};return`<div style="display:flex;justify-content:space-between;font-size:0.875rem;padding:0.25rem 0;gap:0.5rem">
              <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${a(t.name||"")} x${e.quantity}</span>
          ...
          <div class="checkout-step">
            ...
            ${d.length?`<div id="address-list">${d.map(e=>`<label class="card" style="display:flex;align-items:flex-start;gap:1rem;padding:1rem;margin-bottom:0.5rem;cursor:pointer;${s===e.id?"border-color:var(--color-primary)":""}">
              <input type="radio" name="address" value="${e.id}" ${s===e.id?"checked":""} style="margin-top:0.25rem">
              <div><strong>${a(e.recipient_name)}</strong><br><span style="font-size:0.875rem;color:var(--color-gray-500)">${a(e.address_line)}, ${a(e.city)}, ${a(e.province)}${e.reference?"<br>"+a(e.reference):""}${e.phone?"<br>Tel: "+a(e.phone):""}</span></div>
