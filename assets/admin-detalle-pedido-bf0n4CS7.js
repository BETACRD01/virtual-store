...
import{s as m}from"./auth.service-C3JQ1Rkv.js";import{r as E,_ as L}from"./admin.guard-B9R4p4sE.js";import{f as p}from"./currency-BUz-WTj3.js";import{s as a}from"./dom-Dz1M3mpB.js";import{s as w,a as x}from"./notifications-DBxoMB4P.js";const r="/virtual-store/",I=new URLSearchParams(window.location.search),i=I.get("id"),g=["pending","confirmed","preparing","shipped","delivered"],d={pending:"Pendiente",confirmed:"Confirmado",preparing:"Preparando",shipped:"Enviado",delivered:"Entregado",cancelled:"Cancelado"};async function P(){var n;const e=document.getElementById("admin-sidebar");e.innerHTML=`<div class="admin-sidebar">
        ...
        <hr style="border-color:rgba(255,255,255,0.1);margin:1rem 0"><a href="${r}index.html">← Tienda</a><a href="#" id="logout-btn">🚪 Salir</a>
      </div>`,(n=document.getElementById("logout-btn"))==null||n.addEventListener("click",async l=>{l.preventDefault();const{authService:o}=await L(async()=>{const{authService:s}=await import("./auth.service-C3JQ1Rkv.js").then(c=>c.b);return{authService:s}},__vite__mapDeps([0,1]));await o.signOut(),window.location.href=`${r}index.html`})}async function S(){var u,v,f,y,b,h;if(await E(),await P(),!i){document.getElementById("order-detail").innerHTML="<p>Pedido no encontrado.</p>";return}const{data:e,error:n}=await m.from("orders").select("*, order_items(*), address:addresses(*), order_status_history(*), profile:user_id(full_name, email, phone)").eq("id",i).single();if(n){document.getElementById("order-detail").innerHTML=`<p>Error: ${n.message}</p>`;return}const l=t=>t==="delivered"?"badge-success":t==="cancelled"?"badge-danger":"badge-warning",o=g.indexOf(e.status),s=o>=0&&o<g.length-1?g[o+1]:null,c=document.getElementById("order-detail");c.innerHTML=`
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-bottom:1.5rem">
          ...
          <div style="display:flex;gap:0.5rem;align-items:center">
            <span class="badge ${l(e.status)}" style="font-size:1rem">${d[e.status]||e.status}</span>
            ${s?`<button class="btn btn-primary btn-sm" id="next-status-btn">Avanzar a ${d[s]}</button>`:""}
        ...
        ${(y=e.order_status_history)!=null&&y.length?`<div class="card" style="padding:var(--spacing-lg)">
          ...
          ${e.order_status_history.map(t=>`<div style="display:flex;justify-content:space-between;padding:0.5rem 0;border-bottom:1px solid var(--color-gray-100);font-size:0.875rem">
            <span>${d[t.previous_status]||t.previous_status||"—"} → ${d[t.new_status]||t.new_status}</span>
            <span style="color:var(--color-gray-500)">${new Date(t.created_at).toLocaleString("es-MX")}</span>
          </div>`).join("")}
        </div>`:""}
      `,(b=document.getElementById("next-status-btn"))==null||b.addEventListener("click",async()=>{var $;const t=(($=document.getElementById("admin-notes"))==null?void 0:$.value)||"",{error:_}=await m.rpc("update_order_status",{p_order_id:i,p_new_status:s,p_notes:t});if(_){w(_.message);return}x(`Estado actualizado a ${d[s]}.`),window.location.reload()}),(h=document.getElementById("cancel-order-btn"))==null||h.addEventListener("click",async()=>{if(!confirm("¿Cancelar este pedido?"))return;const{error:t}=await m.rpc("cancel_order",{p_order_id:i});if(t){w(t.message);return}x("Pedido cancelado."),window.location.reload()})}S();
