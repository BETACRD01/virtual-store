import{s,g as u,a as w}from"./auth.service-BxJS2Ta1.js";const y="virtual_store_cart";function g(){try{return JSON.parse(localStorage.getItem(y)||"[]")}catch{return[]}}function _(t){localStorage.setItem(y,JSON.stringify(t))}function S(){localStorage.removeItem(y)}const M={async getCart(t){if(!t)return g();const{data:e}=await s.from("carts").select("id").eq("user_id",t).maybeSingle();if(!e)return[];const{data:a}=await s.from("cart_items").select("*, product:products(*)").eq("cart_id",e.id);return a||[]},async addItem(t,e,a=1){const{data:r}=await s.from("products").select("*").eq("id",e).single();if(!r||!r.is_active)throw new Error("Producto no disponible.");if(r.stock<a)throw new Error("Stock insuficiente.");if(!t){const n=g(),o=n.find(m=>m.product_id===e);return o?o.quantity=Math.min(o.quantity+a,r.stock):n.push({product_id:e,quantity:a,unit_price:r.offer_price||r.price,product:r}),_(n),n}let{data:i}=await s.from("carts").select("id").eq("user_id",t).maybeSingle();if(!i){const{data:n}=await s.from("carts").insert({user_id:t}).select().single();i=n}const{data:d}=await s.from("cart_items").select("*").eq("cart_id",i.id).eq("product_id",e).maybeSingle();if(d){const n=Math.min(d.quantity+a,r.stock);await s.from("cart_items").update({quantity:n,unit_price:r.offer_price||r.price}).eq("id",d.id)}else await s.from("cart_items").insert({cart_id:i.id,product_id:e,quantity:a,unit_price:r.offer_price||r.price});return this.getCart(t)},async updateQuantity(t,e,a){if(a<1)return this.removeItem(t,e);if(!t){const i=g(),d=i.find(n=>n.product_id===e);return d&&(d.quantity=a),_(i),i}const{data:r}=await s.from("carts").select("id").eq("user_id",t).single();return await s.from("cart_items").update({quantity:a}).eq("cart_id",r.id).eq("product_id",e),this.getCart(t)},async removeItem(t,e){if(!t){const r=g().filter(i=>i.product_id!==e);return _(r),r}const{data:a}=await s.from("carts").select("id").eq("user_id",t).single();return await s.from("cart_items").delete().eq("cart_id",a.id).eq("product_id",e),this.getCart(t)},async clearCart(t){if(!t)return S(),[];const{data:e}=await s.from("carts").select("id").eq("user_id",t).single();return e&&await s.from("cart_items").delete().eq("cart_id",e.id),[]},async syncLocalCart(t){const e=g();if(!e.length)return this.getCart(t);for(const a of e)try{await this.addItem(t,a.product_id,a.quantity)}catch{}return S(),this.getCart(t)}},x={async getProducts({categoryId:t,search:e,featured:a,offers:r,sort:i,page:d=1,limit:n=12}={}){let o=s.from("products").select("*, category:categories(name, slug)",{count:"exact"}).eq("is_active",!0);t&&(o=o.eq("category_id",t)),e&&(o=o.ilike("name",`%${e}%`)),a&&(o=o.eq("is_featured",!0)),r&&(o=o.not("offer_price","is",null).not("offer_price","eq",0)),i==="price_asc"?o=o.order("price",{ascending:!0}):i==="price_desc"?o=o.order("price",{ascending:!1}):i==="name_asc"?o=o.order("name",{ascending:!0}):i==="name_desc"?o=o.order("name",{ascending:!1}):o=o.order("created_at",{ascending:!1});const m=(d-1)*n,{data:h,error:f,count:p}=await o.range(m,m+n-1);if(f)throw new Error(u(f));return{products:h||[],count:p||0}},async getProductBySlug(t){const{data:e,error:a}=await s.from("products").select("*, category:categories(name, slug)").eq("slug",t).eq("is_active",!0).single();if(a)throw new Error(u(a));return e},async getProductById(t){const{data:e,error:a}=await s.from("products").select("*").eq("id",t).single();if(a&&a.code!=="PGRST116")throw new Error(u(a));return e},async getProductImages(t){const{data:e,error:a}=await s.from("product_images").select("*").eq("product_id",t).order("display_order");if(a)throw new Error(u(a));return e||[]},async getActiveCategories(){const{data:t,error:e}=await s.from("categories").select("*").eq("is_active",!0).order("name");if(e)throw new Error(u(e));return t||[]},async getFeaturedProducts(t=8){const{data:e,error:a}=await s.from("products").select("*, category:categories(name, slug)").eq("is_active",!0).eq("is_featured",!0).limit(t);if(a)throw new Error(u(a));return e||[]},async getOfferProducts(t=8){const{data:e,error:a}=await s.from("products").select("*, category:categories(name, slug)").eq("is_active",!0).not("offer_price","is",null).not("offer_price","eq",0).limit(t);if(a)throw new Error(u(a));return e||[]}};async function k(){var C,L;const t=document.getElementById("main-header");if(!t)return;const e="/virtual-store/",a=await w.getSession(),r=a==null?void 0:a.user,i=r?await w.getProfile(r.id):null,d=(i==null?void 0:i.role)||"customer";let n=0;try{n=(await M.getCart(r==null?void 0:r.id)).reduce((l,B)=>l+(B.quantity||1),0)}catch{}const o=i!=null&&i.full_name?i.full_name.split(" ").map(c=>c[0]).join("").slice(0,2).toUpperCase():((L=(C=r==null?void 0:r.email)==null?void 0:C[0])==null?void 0:L.toUpperCase())||"?",m=r?`<div class="user-menu-wrapper">
